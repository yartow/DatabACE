  ...
  const datesMap = useMemo(() => {
    ...
    allDates?.forEach(d => {
      if (d.date !== null && d.date !== undefined) {
        ...
        const jsDate = new Date(excelEpoch.getTime() + d.date * 86400000);
        const key = `${jsDate.getFullYear()}-${String(jsDate.getMonth() + 1).padStart(2, "0")}-${String(jsDate.getDate()).padStart(2, "0")}`;
        map.set(key, d);
      }
    });
    return map;
